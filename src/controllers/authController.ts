...
export const createUser = checkAsync(async(req:Request,res:Response,next:NextFunction)=>{
...
export const Login = checkAsync(async(req:Request,res:Response,next:NextFunction)=>{
...
interface VRequest extends Request{
    user:any
}

export const verifyMiddle = checkAsync(async(req:VRequest,res:Response,next:NextFunction)=>{
...
export const verifyUser = checkAsync(async(req:VRequest,res:Response,next:NextFunction)=>{
    if(!req.user){
...
//forgot password
export const ForgotPassword = checkAsync(async(req:Request,res:Response,next:NextFunction)=>{
    ...
    const client_url:string = "http://localhost:4000" 
...
export const changePassword = checkAsync(async(req:Request,res:Response,next:NextFunction)=>{
    const password_token:string = req.params.token
...
export const deleteMe = checkAsync(async(req:Request,res:Response,next:NextFunction)=>{
...
export const logout = checkAsync(async(req:Request,res:Response,next:NextFunction)=>{
    res.clearCookie("jwt")
    const id = req.params.id
    res.status(200).json({status:'success'})
})
export const updateMe = checkAsync(async(req:Request,res:Response,next:NextFunction)=>{
    const id = req.params.id
...
export const add_team_member = checkAsync(async(req:VRequest,res:Response,next:NextFunction)=>{
...
export const remove_team_member = checkAsync(async(req:VRequest,res:Response,next:NextFunction)=>{
    const user_role = req.user.role
    if(!(user_role==="admin")){
        return next(new AppError("you have not permission to add team_member",404))
    }
...
export const add_agent = checkAsync(async(req:VRequest,res:Response,next:NextFunction)=>{
    const user_role = req.user.role
...
export const remove_agent = checkAsync(async(req:VRequest,res:Response,next:NextFunction)=>{
    const user_role = req.user.role
    if(!(user_role==="admin" || user_role==="Team_member")){
